...
const PSSEditForm = () => {
  ...
  const getPersonalData = async () => {
    ...
    const response = await axios.get(
      `/api/alternate-investment-funds/${lifeInsuranceEditId}`,
      {
        headers: {
    ...
    let data = response.data.data.InvestmentFund;
  ...
  const lifeInsuranceMutate = useMutation({
    ...
    onSuccess: () => {
      ...
      toast.success("Alternate Investment Fund details added successfully!");
      navigate("/dashboard");
    },
    onError: (error) => {
      console.error("Error submitting profile:", error);
  ...
  if (isError) return <div>Error loading Alternate Investment Fund data</div>;
  return (
