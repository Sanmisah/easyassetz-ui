...
const AlternateInvestmentMainForm = () => {
  ...
  const confirmDelete = async (id) => {
    const response = await axios.delete(
      `/api/alternate-investment-funds/${lifeInsuranceDeleteId}`,
      {
        headers: {
          Authorization: `Bearer ${user.data.token}`,
        },
    ...
    toast.success("Alternate Investment Fund deleted successfully!");
  };
    ...
    <div className="w-[100%] bg-white">
      <div className="flex flex-col w-[100%] ">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold">Alternate Investment Fund</h1>
          <Button onMouseDown={() => navigate("/aif/add")}>
            Add Alternate Investment Fund
          </Button>
          {alertDialog && (
            <DeleteAlert
