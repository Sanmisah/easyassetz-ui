...
const MutualFundsMainForm = () => {
  ...
  const confirmDelete = async (id) => {
    const response = await axios.delete(
      `/api/debentures/${lifeInsuranceDeleteId}`,
      {
        headers: {
          Authorization: `Bearer ${user.data.token}`,
        },
    ...
    toast.success("Debentures deleted successfully!");
  };
    ...
    <div className="w-[100%] bg-white">
      <div className="flex flex-col w-[100%] ">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold">Debentures</h1>
          <Button onMouseDown={() => navigate("/debentures/add")}>
            Add Debentures
          </Button>
          {alertDialog && (
            <DeleteAlert
...
export default MutualFundsMainForm;
