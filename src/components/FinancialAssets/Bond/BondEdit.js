...
const PSSEditForm = () => {
  ...
  const getPersonalData = async () => {
    if (!user) return;
    const response = await axios.get(`/api/bonds/${lifeInsuranceEditId}`, {
      headers: {
        Authorization: `Bearer ${user.data.token}`,
      },
    });

    let data = response.data.data.Bond;
  ...
  const lifeInsuranceMutate = useMutation({
    ...
    onSuccess: () => {
      ...
      toast.success("Bond details added successfully!");
      navigate("/dashboard");
    },
    onError: (error) => {
      console.error("Error submitting profile:", error);
  ...
  if (isError) return <div>Error loading Bond Details data</div>;
  return (
