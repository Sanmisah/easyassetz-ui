...
const WealthAccountMainForm = () => {
  ...
  const confirmDelete = async (id) => {
    const response = await axios.delete(
      `/api/wealth-management-accounts/${lifeInsuranceDeleteId}`,
      {
        headers: {
          Authorization: `Bearer ${user.data.token}`,
        },
    ...
    toast.success("Wealth Management Account deleted successfully!");
  };
    ...
    <div className="w-[100%] bg-white">
      <div className="flex flex-col w-[100%] ">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold">Wealth Management Account</h1>
          <Button onMouseDown={() => navigate("/wealth-account/add")}>
            Add Wealth Management Account
          </Button>
          {alertDialog && (
            <DeleteAlert
            ...
            Benifyciary.map((data) => (
              ...
              >
                <div className="flex flex-col  ml-8">
                  <h1 className="font-bold">{data.wealthManagerName}</h1>
...
export default WealthAccountMainForm;
