...
const BrokingAccountMainForm = () => {
  ...
  const confirmDelete = async (id) => {
    const response = await axios.delete(
      `/api/broking-accounts/${lifeInsuranceDeleteId}`,
      {
        headers: {
          Authorization: `Bearer ${user.data.token}`,
        },
    ...
    toast.success("Broking Account deleted successfully!");
  };
    ...
    <div className="w-[100%] bg-white">
      <div className="flex flex-col w-[100%] ">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold">Broking Account</h1>
          <Button onMouseDown={() => navigate("/broking-account/add")}>
            Add Broking Account
          </Button>
          {alertDialog && (
            <DeleteAlert
            ...
            Benifyciary.map((data) => (
              ...
              >
                <div className="flex flex-col  ml-8">
                  <h1 className="font-bold">{data.brokerName}</h1>
                  <p className="text-sm">{data.brokingAccountNumber}</p>
                </div>
...
export default BrokingAccountMainForm;
