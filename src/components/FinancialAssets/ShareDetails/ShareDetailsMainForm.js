...
const ShareDetailsMainForm = () => {
  ...
  const confirmDelete = async (id) => {
    const response = await axios.delete(
      `/api/share-details/${lifeInsuranceDeleteId}`,
      {
        headers: {
          Authorization: `Bearer ${user.data.token}`,
        },
    ...
    toast.success("Share Details deleted successfully!");
  };
    ...
    <div className="w-[100%] bg-white">
      <div className="flex flex-col w-[100%] ">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold">Share Details</h1>
          <Button onMouseDown={() => navigate("/share-details/add")}>
            Add Share Details
          </Button>
          {alertDialog && (
            <DeleteAlert
...
export default ShareDetailsMainForm;
