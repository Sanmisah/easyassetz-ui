...
const AlternateInvestmentMainForm = () => {
  ...
  const confirmDelete = async (id) => {
    const response = await axios.delete(
      `/api/portfolio-managements/${lifeInsuranceDeleteId}`,
      {
        headers: {
          Authorization: `Bearer ${user.data.token}`,
        },
    ...
    toast.success("Portfolio Management Services deleted successfully!");
  };
    ...
    <div className="w-[100%] bg-white">
      <div className="flex flex-col w-[100%] ">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold">Portfolio Management Services</h1>
          <Button onMouseDown={() => navigate("/pms/add")}>
            Add Portfolio Management Services
          </Button>
          {alertDialog && (
            <DeleteAlert
            ...
            Benifyciary.map((data) => (
              ...
              >
                <div className="flex flex-col  ml-8">
                  <h1 className="font-bold">{data.fundName}</h1>
                  <p className="text-sm">{data.folioNumber}</p>
                </div>
...
export default AlternateInvestmentMainForm;
