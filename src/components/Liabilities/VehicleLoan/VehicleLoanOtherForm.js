...
const VehicleLoanOtherForm = () => {
  ...
  const loanMutate = useMutation({
    mutationFn: async (data) => {
      const response = await axios.post(`/api/vehicle-loans`, data, {
        headers: {
    ...
    onSuccess: () => {
      ...
      toast.success("Vehicle Loan added successfully!");
      navigate("/dashboard");
    },
    onError: (error) => {
          ...
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-2">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <CardDescription>
                  Fill out the form to add a new Vehicle Loan.
                </CardDescription>
              </div>
            </div>
          </div>
...
export default VehicleLoanOtherForm;
