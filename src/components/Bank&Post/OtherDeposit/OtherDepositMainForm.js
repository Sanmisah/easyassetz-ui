...
const OtherDepositsMainForm = () => {
  ...
  const confirmDelete = async (id) => {
    const response = await axios.delete(
      `/api/other-deposites/${lifeInsuranceDeleteId}`,
      {
        headers: {
          Authorization: `Bearer ${user.data.token}`,
        },
    ...
    toast.success("Other Deposite deleted successfully!");
  };
            ...
            Benifyciary.map((data) => (
              ...
              >
                <div className="flex flex-col  ml-8">
                  <h1 className="font-bold">{data.fdNumber}</h1>
...
export default OtherDepositsMainForm;
