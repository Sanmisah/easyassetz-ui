...
const PSADMainForm = () => {
  ...
  const confirmDelete = async (id) => {
    const response = await axios.delete(
      `/api/post-saving-account-details/${lifeInsuranceDeleteId}`,
      {
        headers: {
          Authorization: `Bearer ${user.data.token}`,
        },
    ...
    toast.success("Post Saving Account Details deleted successfully!");
  };
            ...
            Benifyciary.map((data) => (
              ...
              >
                <div className="flex flex-col  ml-8">
                  <h1 className="font-bold">{data.accountNumber}</h1>
                  <p className="text-sm">{data.postOfficeBranch}</p>
                </div>
