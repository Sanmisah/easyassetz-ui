...
const Crypto = () => {
  ...
  const confirmDelete = async (id) => {
    const response = await axios.delete(
      `/api/cryptos/${lifeInsuranceDeleteId}`,
      {
        headers: {
          Authorization: `Bearer ${user.data.token}`,
        },
    ...
    toast.success("Crypto details deleted successfully!");
  };
    ...
    <div className="w-[100%] bg-white">
      <div className="flex flex-col w-[100%] ">
        <div className="flex items-center justify-between">
          ...
          <Button onMouseDown={() => navigate("/crypto/add")}>
            Add Crypto
          </Button>
          {alertDialog && (
            <DeleteAlert
            ...
            Benifyciary.map((data) => (
              ...
              >
                <div className="flex flex-col  ml-8">
                  <h1 className="font-bold">{data.cryptoWalletType}</h1>
                  <p className="text-sm">{data.cryptoWalletAddress}</p>
                </div>
